  ...
  const runBatch = async () => {
    ...
    for (let fi = 0; fi < files.length; fi++) {
      ...
      try {
        ...
        let outCv = cv;
        if (contrast !== 0 || saturation !== 0) {
          outCv = document.createElement("canvas"); outCv.width = w; outCv.height = h;
        ...
        let blob, ext;
        if (batchFmt === "tiff") {
          blob = encodeTIFF(outCv); ext = "tif";
        } else if (batchFmt === "png") {
          blob = await new Promise(r => outCv.toBlob(r, "image/png")); ext = "png";
        } else {
          blob = await new Promise(r => outCv.toBlob(r, "image/jpeg", 0.95)); ext = "jpg";
          if (blob && exifSeg) {
            try {
